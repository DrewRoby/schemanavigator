...
def reprocess_file(request, pk):
    """Re-process a file to create a new schema version or attempt detection again"""
    datasource = get_object_or_404(DataSource, pk=pk)
    ...
    if request.method == 'POST':
        ...
        if schema_exists:
            ...
            # Process the new datasource
            success = process_file(new_datasource)

            if success:
                messages.success(request, f'Created new schema version (v{new_datasource.schema_version}) for {new_datasource.original_filename}')
                return redirect('datasource_detail', pk=new_datasource.pk)
            else:
                # If processing failed, delete the new datasource to avoid duplicates
                new_datasource.delete()
                messages.error(request, f'Failed to create new schema version for {datasource.original_filename}')
        else:
            # Simply try to process the file again
            success = process_file(datasource)

            if success:
                messages.success(request, f'Successfully analyzed {datasource.original_filename}')
            else:
                messages.error(request, f'Failed to analyze {datasource.original_filename}')
